...
fn report_stats(tag: &str, start: Instant, ops: u64, len: usize) {
    tikv_util::sys::record_global_memory_usage();
    let rss = tikv_util::sys::get_global_memory_usage();
    ...
    if let Some(stats) = fetch_jemalloc_stats() {
        println!(
            "tag={tag} elapsed_s={elapsed:.1} ops={ops} ops_s={ops_per_sec:.1} len={len} rss_mb={rss_mb:.1} jemalloc_alloc_mb={alloc_mb:.1} jemalloc_active_mb={active_mb:.1} jemalloc_resident_mb={resident_mb:.1} jemalloc_retained_mb={retained_mb:.1}",
            rss_mb = bytes_to_mb(rss),
            alloc_mb = bytes_to_mb(stats.allocated),
            active_mb = bytes_to_mb(stats.active),
            resident_mb = bytes_to_mb(stats.resident),
            retained_mb = bytes_to_mb(stats.retained),
        );
    } else {
        println!(
            "tag={tag} elapsed_s={elapsed:.1} ops={ops} ops_s={ops_per_sec:.1} len={len} rss_mb={rss_mb:.1} jemalloc=NA",
            rss_mb = bytes_to_mb(rss),
        );
    }
}
